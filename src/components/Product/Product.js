                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-8 md:px-0">   
                        {
                            prop.data.sort(
                                ...
                                <div key={product.id} className="bg-gray-50 transition-all ease-in-out duration-400 overflow-hidden border border-gray-300 bg-amber-100 shadow-md text-gray-700 hover:scale-105 rounded-lg md:rounded-xl p-3 md:p-6 min-h-max mx-2 my-2 md:mx-0">
                                    <div className="text-justify text-sm h-full flex flex-col justify-between">
                                        <div className='relative'>
                                            {product.attributes.gia_sanpham && <div className='absolute top-2 right-2 shadow-md text-secondary-600 py-1 px-3 md:py-1 md:px-4 bg-gray-100 text-base md:text-lg font-bold rounded-full tracking-wide'>{product.attributes.gia_sanpham}</div>}
                                        ...
                                        <div className="flex justify-center mt-4">
                                                <Link to={`/san-pham/${product.attributes.slug}`} className="text-white bg-primary-700 hover:bg-secondary-600 inline-flex items-center justify-center px-3 md:px-3 py-2 text-md md:text-base shadow-xl rounded-full group">
                                                    Xem chi tiết
